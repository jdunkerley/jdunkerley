...
This means that at some point during the period, the fund experienced a drawdown of 12.8%, the largest drop from peak to trough in the dataset.
...
Let's again convert the price series into an index series. We can do this using the expression `[Price]/(first([Price]))` to create an index that starts at 1 on the first day (after sorting the data by date) and increases with price changes.
...
Finally, we can plot the fund returns against the benchmark returns to visually inspect the relationship between the two. This can be done by selecting the three columns (`Date`, `Value` and `BenchmarkIndex`) and then choosing the Scatter Plot option in the visualisation menu.

![Scatter plot of fund versus index](image-10.png)

## Computing Excess Returns and Sharpe Ratio

The next step in our analysis is to compare the fund's performance against a risk-free rate to compute excess returns and a Sharpe ratio. The risk-free rate represents the amount of interest you can earn on an investment with zero risk, such as in a bank account or in a government bond. For this case, I will use the Bank of England's base rate as a proxy for the risk-free rate.
...
Using `Data.fetch` with this URI will allow us to download the data directly into Enso. You need to specifiy to the format as `..Delimited` so that Enso parses the result into a table. The API reports the content type as `application/csv`, but this is not the standard MIME type for a CSV (`text/csv`), so we need to specify the format explicitly. We also need to parse the date column - as the format from the API is `dd MMM yyyy` as opposed to a standard ISO format. Then we can join this risk-free rate to the fund data, in a similar way to how we joined the benchmark data.

The Bank of England's base rate is an annual rate, so we need to convert it to a daily rate by dividing by 365 as the UK convention (`Actual/365`) and then times the number of days since the previous row to get the daily risk-free return. The expression for this is `([RiskFree]/100)*[Days Since Previous]/365`. The `ExcessReturn` can then be computed by subtracting the risk-free return from the fund return.
...
In this post, we have looked at how to take the index and the return series we created and compute drawdown statistics, as well as how to join the fund data to a benchmark and compute the correlation. 

Finally, we also looked at how to compute excess returns relative to a risk-free rate and a Sharpe ratio to measure the risk-adjusted performance of the fund.

Along the way, we have seen how to use various functions in Enso, such as `running`, `fill_nothing`, `merge`, and `compute_bulk` to perform these calculations. We've also touched on various ways to import, cleanse and prepare data for analysis in Enso, as well as how to create visualisations to help understand the relationships between different series.
